...
# Deficit/surplus applied to maintenance TDEE, matching the "roughly 15-20% deficit" /
# "roughly 10-15% surplus" language in the same nutrition docs.
CALORIE_ADJUSTMENT = {"CUT": 0.825, "MAINTAIN": 1.0, "BULK": 1.125}

# No activity-level field is collected, so this assumes "lightly-to-moderately active" -
# a reasonable default for someone using a workout-tracking app, rather than sedentary.
ACTIVITY_MULTIPLIER = 1.45


def _daily_targets(height_cm, weight_kg, age, sex, goal, bmi_cat):
    """Daily calorie + protein targets, computed directly from the user's own stats rather
    than left for the model to invent - so portion sizes actually scale with the person
    instead of being a fixed "here's a chicken breast" every time.

    Calories use the Mifflin-St Jeor equation for BMR, then a fixed activity multiplier
    (see ACTIVITY_MULTIPLIER - no activity-level field exists to do better than a fixed
    assumption there) to estimate maintenance TDEE.

    bmi_underweight.txt/bmi_overweight_obese.txt both say a goal-driven deficit/surplus is
    inappropriate at those BMI extremes - that caution is applied here too, not just
...
def _build_workout_prompt(height_cm, weight_kg, age, sex, goal, bmi, category, context_chunks):
    context_block = "\n\n".join(context_chunks)
    return f"""You are a knowledgeable, encouraging fitness coach. Build a full 7-day WORKOUT plan \
(no meals - a separate step handles nutrition) for a user with these stats:
- Height: {height_cm} cm, Weight: {weight_kg} kg, Age: {age}, Sex: {sex.title()}, BMI: {bmi:.1f} ({category})
- Goal: {goal.title()}

Use the following reference material as the basis for your plan - stay consistent with the rep \
ranges and split structure it describes:
...
specific day; otherwise leave it as an empty string.
...
def _build_meal_prompt(height_cm, weight_kg, age, sex, goal, bmi, category, diet_category, context_chunks, calories, protein_g):
    context_block = "\n\n".join(context_chunks)
    return f"""You are a knowledgeable, encouraging nutrition coach. Build a full 7-day MEAL plan \
(no workouts - a separate step handles training) for a user with these stats:
- Height: {height_cm} cm, Weight: {weight_kg} kg, Age: {age}, Sex: {sex.title()}, BMI: {bmi:.1f} ({category})
- Goal: {goal.title()}
- Diet: {DIET_TITLES[diet_category]}
- Estimated daily target: ~{calories} kcal, ~{protein_g}g protein (derived from this user's own \
stats and goal - use it to size portions, not as a rigid exact number)

Use the following reference material as the basis for your plan - stay consistent with the \
nutrition/diet guidance it describes:
...
- Give a short meal plan for each day: breakfast, lunch, dinner, and one snack. Every single food \
item in every meal MUST include a specific, realistic quantity (grams, ounces, cups, or a count - \
e.g. "150g grilled chicken breast", "1 cup cooked rice", "2 whole eggs", "30g almonds"). Never list \
a food with no amount. Size the day's total portions to roughly reach the calorie and protein target \
above, distributed across meals following whatever meal pattern (number of meals, snack frequency) \
the reference material describes for this goal.
- Every day of the week must appear, in order (Monday-Sunday).
- Add one short "notes" string per day only if there's something genuinely useful to flag for that \
...
def generate_meal_plan(height_cm, weight_kg, age, sex, goal, diet_category):
    ...
    calories, protein_g = _daily_targets(height_cm, weight_kg, age, sex, goal, category)

    context_chunks, sources = retrieve_meal_context(goal, category, diet_category)
